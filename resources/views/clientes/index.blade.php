    ...
    <form id="filter-form" method="GET" action="{{ route('clientes.index') }}" class="flex flex-wrap items-center gap-4">
        ...
        <div class="flex-1 min-w-[300px]">
            <div class="relative">
                ...
                <input type="text" name="buscar" value="{{ $busqueda }}" placeholder="Buscar por nombre..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-600 focus:border-gray-600 outline-none">
            </div>
        </div>
        ...
        <a href="{{ route('clientes.create') }}" class="ml-auto inline-flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                @forelse($clientes as $cliente)
                    <tr class="hover:bg-gray-100 transition-colors cursor-pointer {{ !$cliente['activo'] ? 'opacity-50' : '' }}"
